...
Design reference for the core rules and algorithms behind a group trip-planning app, written to be built from scratch and to work for **any trip** — any destination, dates, timezone, or group size — not tied to one specific journey. Treat every rule below as a requirement for the new implementation, not as a description of pre-existing code.

> Terminology: "stop" and "event" are used interchangeably for a single itinerary entry (one record with a category, an optional day/time, and so on) — "event" leans toward the underlying data row, "stop" toward the domain/UI concept. Same thing, two names.
